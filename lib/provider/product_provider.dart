...
class ProductProvider extends ChangeNotifier {
  ...
  void initRecentProductList(String id, BuildContext context) async {
    ApiResponse apiResponse = await productRepo.getRecentProductList(id);
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
      _recentProductList = [];
      apiResponse.response.data.forEach(
          (product) => _recentProductList.add(Product.fromJson(product)));
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    notifyListeners();
  ...
  List<Product> get otherProductList => _relatedProductList;

  void initOtherProductList(String id, BuildContext context) async {
    ApiResponse apiResponse = await productRepo.getOtherProductList(id);
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
      _otherProductList = [];
      apiResponse.response.data.forEach(
          (product) => _otherProductList.add(Product.fromJson(product)));
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    notifyListeners();
